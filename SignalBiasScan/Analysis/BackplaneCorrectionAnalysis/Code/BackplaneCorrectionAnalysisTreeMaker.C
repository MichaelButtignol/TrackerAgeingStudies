#define BackplaneCorrectionAnalysisTreeMaker_cxx

#include "BackplaneCorrectionAnalysisTreeMaker.h"
...
// Main method //
//-------------//
void BackplaneCorrectionAnalysisTreeMaker::Loop()
{
...
} //end of BackplaneCorrectionAnalysisTreeMaker::Loop()
...
void BackplaneCorrectionAnalysisTreeMaker::FillHitInfo(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle,
...
} // end of BackplaneCorrectionAnalysisTreeMaker::FillHitInfo
...
void BackplaneCorrectionAnalysisTreeMaker::FillHistos(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle,
...
} // end of BackplaneCorrectionAnalysisTreeMaker::FillHistos
...
void BackplaneCorrectionAnalysisTreeMaker::FitHistos(std::map<ULong64_t , std::vector<TH1F*> > &HistSoN, string output_file, 
 std::vector< TH1F* > commonHistos,std::vector< TH2F* > additionaryHistos, std::map<ULong64_t, TProfile* > Monitors)
{
	...
	for(itMon=Monitors.begin(); itMon!=Monitors.end(); itMon++)
	{
  	...
  	//myFile->Write();
  	myFile->Close();
} // end of BackplaneCorrectionAnalysisTreeMaker::FitHistos


void BackplaneCorrectionAnalysisTreeMaker::FitProfiles(std::map<ULong64_t , std::vector<TProfile*> > &ProfVsAngle, string output_file)
{
	TFile * myFile = new TFile(output_file.c_str(), "recreate");
	...
	for(std::map<ULong64_t , std::vector<TProfile*> >::iterator iter = ProfVsAngle.begin(); iter != ProfVsAngle.end(); ++iter)
	{
  	...
  	//myFile->Write();
  	myFile->Close();
} // end of BackplaneCorrectionAnalysisTreeMaker::FitProfiles
