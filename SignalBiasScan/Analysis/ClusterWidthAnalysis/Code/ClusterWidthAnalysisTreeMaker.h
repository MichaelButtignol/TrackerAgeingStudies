...
#ifndef ClusterWidthAnalysisTreeMaker_h
#define ClusterWidthAnalysisTreeMaker_h
...
class ClusterWidthAnalysisTreeMaker {
public :
   ...
   ClusterWidthAnalysisTreeMaker(TTree *tree=0, std::string file="Run.txt", int subdet=1, bool timestamp=false,
    int angle = 0, int width = 0, int onstrip = 0 );
   virtual ~ClusterWidthAnalysisTreeMaker();
   ...
   void FillHistos(std::map< ULong64_t , std::vector<TH1F*> > &, std::map< ULong64_t , std::vector<TProfile*> > &,
                   std::vector<TreeFullHit> &, int , bool , std::vector< TH1F* >, std::map<ULong64_t, TProfile* > );
...
#ifdef ClusterWidthAnalysisTreeMaker_cxx
ClusterWidthAnalysisTreeMaker::ClusterWidthAnalysisTreeMaker(TTree *tree, std::string file,
...
ClusterWidthAnalysisTreeMaker::~ClusterWidthAnalysisTreeMaker()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t ClusterWidthAnalysisTreeMaker::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t ClusterWidthAnalysisTreeMaker::LoadTree(Long64_t entry)
{
...
void ClusterWidthAnalysisTreeMaker::Init(TTree *tree)
{
...
Bool_t ClusterWidthAnalysisTreeMaker::Notify()
{
...
void ClusterWidthAnalysisTreeMaker::Show(Long64_t entry)
{
...
Int_t ClusterWidthAnalysisTreeMaker::Cut(Long64_t entry)
{
...
#endif // #ifdef ClusterWidthAnalysisTreeMaker_cxx
